...
class CreateServer(commands.Cog, name="Create Server Stuff", command_attrs=dict(hidden=True)):
    ...
    @commands.command(aliases=["build"], help="| Sets up server with roles and permissions.", usage="Optional: Server Name")
    @commands.has_permissions(manage_guild=True)
    async def create_server(self, ctx, *, serverName: str = None):
        ...
        mainOverwrites = {
            guild.default_role: discord.PermissionOverwrite(read_messages=True, send_messages=True, add_reactions=True, connect=True, speak=True)
        }

        # defaultRole = discord.utils.get(guild.roles, guild.default_role)
        await guild.default_role.edit(permissions=discord.Permissions(mention_everyone=False, send_tts_messages=False))
        ...
        await guild.create_text_channel(name="high-staff", topic="Staff announcements will go here", category=staffCategory)
        ...
        await guild.create_text_channel(name="edited-deleted-logs", category=logsCategory)
        ...
        await guild.edit(name=serverName, verification_level=discord.VerificationLevel.medium)
