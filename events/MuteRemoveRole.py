...
class MuteRemove(Cog):
    ...
    async def on_member_update(self, before, after):
        # Muted People
        check_guild_in_db = await self.bot.pool.fetchval(
            f"SELECT guild_id FROM guild_mutes WHERE guild_id = $1", before.guild.id
        )
        is_user_muted = await self.bot.pool.fetchval(
        ...
        if role in before.roles and role not in after.roles:
            await self.bot.pool.execute(
                "DELETE FROM guild_mutes WHERE guild_id = $1 AND member_id = $2",
                before.guild.id,
                before.id,
            )
